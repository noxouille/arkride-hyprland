...
# ============================================================================
# Theme Generator - Create a new theme from Arkride template
# ============================================================================
#
# WORKFLOW:
# ---------
...
#
# 2. Run this script:
#      ./generate-theme.sh --name "mytheme" --colors mytheme.scss --output ~/repo/mytheme-hyprland
#
# 3. Test the generated theme:
#      cd ~/repo/mytheme-hyprland
#      ./install.sh
#
...
#
# Example: Arkride's $agnes (coral) is mapped via $secondary.
#          Your theme's $character-name can be anything, as long as
#          you set $secondary to point to it.
#
# ============================================================================
# Usage: ./generate-theme.sh --name "mytheme" --colors my-colors.scss --output ~/repo/mytheme-hyprland
# ============================================================================
...
usage() {
    ...
    echo ""
    echo "Example:"
    echo "  $0 --name mytheme --colors ~/my-colors.scss --output ~/repo/mytheme-hyprland"
    exit 1
}
...
# ============================================================================
# Semantic role mapping - maps Arkride roles to new theme equivalents
# ============================================================================
...
#
# Arkride semantic roles (from _colors.scss):
...
# Build a mapping from Arkride hex values to new theme hex values
...
if [[ ${#missing_colors[@]} -gt 0 ]]; then
    ...
    echo ""
    warn "These will keep their original Arkride values."
    echo ""
fi
...
if [[ -d "$OUTPUT_DIR/arkride-gallery" ]]; then
    mv "$OUTPUT_DIR/arkride-gallery" "$OUTPUT_DIR/${THEME_NAME_LOWER}-gallery"
    # Update python script name
    if [[ -f "$OUTPUT_DIR/${THEME_NAME_LOWER}-gallery/arkride-gallery.py" ]]; then
        mv "$OUTPUT_DIR/${THEME_NAME_LOWER}-gallery/arkride-gallery.py" \
           "$OUTPUT_DIR/${THEME_NAME_LOWER}-gallery/${THEME_NAME_LOWER}-gallery.py"
    fi
fi

if [[ -d "$OUTPUT_DIR/extras/nvim/arkride.nvim" ]]; then
    mv "$OUTPUT_DIR/extras/nvim/arkride.nvim" "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim"
    # Rename colors file
    if [[ -f "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/colors/arkride.lua" ]]; then
        mv "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/colors/arkride.lua" \
           "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/colors/${THEME_NAME_LOWER}.lua"
    fi
    # Rename lua module directory
    if [[ -d "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/lua/arkride" ]]; then
        mv "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/lua/arkride" \
           "$OUTPUT_DIR/extras/nvim/${THEME_NAME_LOWER}.nvim/lua/${THEME_NAME_LOWER}"
    fi
fi

if [[ -d "$OUTPUT_DIR/themes/Arkride" ]]; then
    mv "$OUTPUT_DIR/themes/Arkride" "$OUTPUT_DIR/themes/${THEME_NAME_CAPITALIZED}"
fi

# Rename VSCode theme file
if [[ -f "$OUTPUT_DIR/extras/vscode/arkride-color-theme.json" ]]; then
    mv "$OUTPUT_DIR/extras/vscode/arkride-color-theme.json" \
       "$OUTPUT_DIR/extras/vscode/${THEME_NAME_LOWER}-color-theme.json"
    # Update package.json to reference the new filename
    sed -i "s/arkride-color-theme.json/${THEME_NAME_LOWER}-color-theme.json/g" \
        "$OUTPUT_DIR/extras/vscode/package.json"
fi

# Update install.sh banner text (remove Arkride-specific description)
...
# ============================================================================
# Clean up Arkride-specific content in README
# ============================================================================
...
Generated from [arkride-hyprland](https://github.com/your-username/arkride-hyprland) template.
EOF
