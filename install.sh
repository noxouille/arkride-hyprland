...
# ============================================================================
# Arkride Hyprland Theme - Installation Script
# ============================================================================
...
THEME_NAME="arkride"
GTK_THEME_NAME="Arkride-BL-MB-Dark"

echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║           Arkride Hyprland Theme - Installation                  ║"
echo "║      Inspired by Kuro no Kiseki / Trails through Daybreak        ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo ""
...
# ============================================================================
# Config files → ~/.config/colorschemes/arkride/
# ============================================================================
install_colorscheme() {
...
# ============================================================================
# GTK Theme → ~/.themes/Arkride-BL-MB-Dark/
# ============================================================================
install_gtk() {
  ...
  cp "${SCRIPT_DIR}/themes/Arkride/index.theme" ~/.themes/${GTK_THEME_NAME}/
  cp "${SCRIPT_DIR}/themes/Arkride/gtk-3.0/gtk.css" ~/.themes/${GTK_THEME_NAME}/gtk-3.0/

  # Update index.theme with correct name
  sed -i "s/Name=Arkride/Name=${GTK_THEME_NAME}/" ~/.themes/${GTK_THEME_NAME}/index.theme
  sed -i "s/GtkTheme=Arkride/GtkTheme=${GTK_THEME_NAME}/" ~/.themes/${GTK_THEME_NAME}/index.theme
  sed -i "s/MetacityTheme=Arkride/MetacityTheme=${GTK_THEME_NAME}/" ~/.themes/${GTK_THEME_NAME}/index.theme
...
# ============================================================================
# Arkride Gallery → ~/.local/share/arkride-gallery/
# ============================================================================
install_gallery() {
  info "Installing Arkride Gallery..."

  mkdir -p ~/.local/share/arkride-gallery

  # Remove old installation if exists
  rm -rf ~/.local/share/arkride-gallery/*

  # Copy gallery files
  cp -r "${SCRIPT_DIR}/arkride-gallery/"* ~/.local/share/arkride-gallery/

  success "Arkride Gallery installed to ~/.local/share/arkride-gallery/"
  echo ""
  info "Usage: python ~/.local/share/arkride-gallery/arkride-gallery.py [portrait|side]"
}

# ============================================================================
# Neovim → ~/.local/share/nvim/themes/arkride.nvim/
# ============================================================================
install_nvim() {
  ...
  # Remove old installation if exists
  rm -rf ~/.local/share/nvim/themes/arkride.nvim

  # Copy neovim theme
  cp -r "${SCRIPT_DIR}/extras/nvim/arkride.nvim" ~/.local/share/nvim/themes/

  success "Neovim theme installed to ~/.local/share/nvim/themes/arkride.nvim/"
  echo ""
  info "Add to ~/.config/nvim/lua/plugins/colorscheme.lua:"
  echo ""
  echo '  {'
  echo '    dir = "~/.local/share/nvim/themes/arkride.nvim",'
  echo '    name = "arkride",'
  echo '    lazy = false,'
  echo '    priority = 1000,'
  echo '  },'
...
install_vscode() {
  ...
  # Create VSIX package
  mkdir -p /tmp/arkride-vsix/extension
  cp "${SCRIPT_DIR}/extras/vscode/package.json" /tmp/arkride-vsix/extension/
  cp "${SCRIPT_DIR}/extras/vscode/arkride-color-theme.json" /tmp/arkride-vsix/extension/

  cd /tmp/arkride-vsix
  zip -r /tmp/arkride-theme.vsix extension >/dev/null
  cd - >/dev/null
  rm -rf /tmp/arkride-vsix

  # Try to install
  if command -v codium &>/dev/null; then
    codium --install-extension /tmp/arkride-theme.vsix
    success "VSCodium theme installed!"
    info "Activate: Ctrl+K Ctrl+T → Select 'Arkride'"
  elif command -v code &>/dev/null; then
    code --install-extension /tmp/arkride-theme.vsix
    success "VS Code theme installed!"
    info "Activate: Ctrl+K Ctrl+T → Select 'Arkride'"
  else
    success "VSIX package created at /tmp/arkride-theme.vsix"
    info "Install manually: codium --install-extension /tmp/arkride-theme.vsix"
  fi

  rm -f /tmp/arkride-theme.vsix
}
...
show_menu() {
  ...
  echo "  4) Arkride Gallery (~/.local/share/arkride-gallery/)"
